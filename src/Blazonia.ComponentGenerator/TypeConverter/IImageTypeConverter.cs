...
internal class IImageTypeConverter : BaseTypeConverter
{
    ...
    public override string GetHandleValueProperty(string ComponentTypeName, string AvaloniaPropertyName, string propName)
    {
        return $@"                case nameof({propName}):
                    ...
                    {{
                        ...
                        {{
                            NativeControl.{AvaloniaPropertyName} = new global::Avalonia.Media.Imaging.Bitmap(global::Avalonia.Platform.AssetLoader.Open(new Uri({propName}.AsT1)));
                        }}
                    }}
                    break;
";
    ...
    {
        return typeSymbol.GetFullName() == "Avalonia.Media.IImage";
    }
}